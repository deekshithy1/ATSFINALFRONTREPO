...
const handleClick = async (e) => {
  ...
  try {
    const response = await axiosInstance.get(
      `/vehicles/vehiclefromparivahan/${vehicleData.bookingId}`
    );
    console.log("Vehicle fetched:", response.data);

    setVehicleData(prev => ({
      ...prev,
      regnNo: response.data.vehicleNumber,
      chassisNo: response.data.chasisNumber,
      engineNo: response.data.engineNumber
    }));
  } catch (err) {
    console.error("Error getting Details:", err.response?.data || err.message);
    ...
    <div className="fixed inset-0 bg-black/80   z-50 flex justify-center items-center">
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 w-full max-w-md relative">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
          {[
            { label: "Booking ID", name: "bookingId",buttonLabel:"Get vehicleDetails" },
            { label: "Registration Number", name: "regnNo" },
            { label: "Engine Number", name: "engineNo" },
            { label: "Chassis Number", name: "chassisNo" },
          ].map(({ label, name,buttonLabel }) => (
            <div key={name}>
              <label className="block mb-1 font-medium text-gray-700">{label}</label>
              {
                label==="Booking ID" ? (
                <input type="text" name={name} value={vehicleData[name]} onChange={handleChange} required  
