...
//                 <th className="px-5 py-3 sm:hidden block">Engine No</th>
//                 <th className="px-5 py-3 sm:hidden block">Chassis No</th>
//                 <th className="px-5 py-3 sm:hidden block">Lane Entry</th>
...
//                   <td className="px-5 py-4 sm:hidden block">{vehicle.engineNo}</td>
//                   <td className="px-5 py-4 sm:hidden block">{vehicle.chassisNo}</td>
//                   <td className="px-5 py-4 sm:hidden block">
...
const Vehicles = () => {
  ...
  useEffect(() => {
    const loadVehicles = async () => {
      if (selectedDate === dayjs().format("YYYY-MM-DD")) {
        ...
        const data = await fetchVehiclesByDate(selectedDate);
        useVehicleStore.setState({ vehicles: data });
      }
    };
  ...
  useEffect(() => {
    const applyFilter = async () => {
      ...
      if (user?.role === "MVI") {
        filteredList = await fetchVehicles();
      }
  ...
  const handleClick = async (regnNo) => {
    try {
      await axiosInstance.post("/tests/start", { regnNo });
    } catch (error) {
      console.error(error);
    }
  };
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-3">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          ...
          <input
            type="text"
            placeholder="Search by Booking ID..."
            className="border border-gray-300 px-4 py-2 rounded-md text-sm w-full sm:w-64"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
          ...
          {user?.role === "ATS_ADMIN" && (
            <button
              onClick={() => setShowModal(true)}
              className="flex items-center justify-center gap-2 bg-blue-600 text-white px-5 py-2 rounded-md text-sm font-medium hover:bg-blue-700"
            >
              <Plus size={16} /> Add Vehicle
            </button>
          )}
          ...
          <table className="min-w-[900px] w-full text-sm text-left text-gray-700">
            ...
            <thead className="bg-gray-100 text-xs uppercase text-gray-500">
              ...
              <tr>
                ...
                <th className="px-5 py-3 hidden md:table-cell">Engine No</th>
                <th className="px-5 py-3 hidden md:table-cell">Chassis No</th>
                <th className="px-5 py-3 hidden md:table-cell">Lane Entry</th>
                    ...
                    <td className="px-5 py-4">
                      ...
                      <div className="flex flex-wrap justify-center gap-2">
                        ...
                        {vehicle.status === "IN_PROGRESS" && (
                          <>
                            ...
                            {(user?.role === "ATS_ADMIN" ||
                              ...
                              <button
                                onClick={() =>
                                  handleFunctionalTest(vehicle.bookingId)
                                }
                        ...
                        {vehicle.status === "COMPLETED" && (
                          <button
                            onClick={() =>
                              handleApproval(vehicle.bookingId)
                            }
                        ...
                        {vehicle.status === "APPROVED" && (
                          <button
                            onClick={() =>
                              handleReport(vehicle.bookingId)
                            }
