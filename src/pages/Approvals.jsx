...
//                   }
//                   { user?.role==="MVI"&&
// <button className='bg-red-500 text-white  p-4 rounded-2xl' onClick={handleSubitNIC} >Send To NIC</button> 
//                   }
//                   </td>
//                 </tr>
//               ))}
...
const Approvals = () => {
  ...
  const [vehicles, setVehicles] = useState([]);
  const [selectedVehicle, setSelectedVehicle] = useState(null);
  const [testInstances, setTestInstances] = useState(null);

  const { user } = useAuthStore();

  const fetchgetreadyforApproval = useVehicleStore(
    (state) => state.getVehiclesReadyForApproval
  );
  const vehicless = useVehicleStore((state) => state.readyVehicles);
  ...
  /* 🔴 FIX: accept vehicle as argument */
  const handleSubitNIC = async (vehicle) => {
    if (!vehicle) return;

    const regnNo = vehicle.regnNo;
              ...
              {vehicles && vehicles.map((v) => (
                <tr key={v.bookingId} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-4">
                    {v.laneEntryTime?.slice(0, 16).replace("T", " ")}
                  </td>
                  <td className="px-5 py-4">
                    {v.testInstances[0]?.submittedBy?.name}
                  </td>
                  <td className="px-5 py-4">{v.status}</td>
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">
                      {testInstances?.submittedBy?.name}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Status:</span>
          ...
          {user?.role !== "MVI" && (
            <div>
              <button
                ...
                onClick={() => handleSubitNIC(selectedVehicle)}
              >
                Send To NIC
              </button>
            </div>
